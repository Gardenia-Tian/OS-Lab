...
extern schedule
extern main
extern pn ;保存当前进程控制块的指针->程序并发用的->schedule里面要修改
extern preg ;中断用的save
extern reg
extern regaddr 
extern PCBList ;进程控制块表
extern name
extern CurrentPCBid
extern ProgramCnt
extern getbuf
...
_clear:
	...
	ret
;==========================================================================================================
;void _initest()      先把所有的程序全部加载到内存  0x00008214
;==========================================================================================================	
_initest:	
	push es
	push si
...
tokernel:
	...
	mov  ax,cs 
	mov  ds,ax 
	mov  word[CurrentPCBid],0
	mov  word[ProgramCnt],0
	mov  word[cnt],0
	mov  ax, reg
	mov  word[pn], ax ;让pn指向保存内核现场的变量地址
	pop ax 
	pop ds 
...
datadef:
	...
	color db 02h
	cnt   dw 0          ;现在用来表示时间片计数，每个进程一个时间片，总的时间片到了200就返回
	chara db alltype
	char1  db '|'
	char2  db '\\'
	char3  db '-'
